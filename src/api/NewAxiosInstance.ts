...
const newApiManager: AxiosInstance = create({
  baseURL: process.env.EXPO_PUBLIC_NEW_SERVER_URL,
  timeout: 500000,
  headers: {
...
newApiManager.interceptors.request.use(
...
newApiManager.interceptors.response.use(
    ...
    if (errorResponse?.data) {
      if (errorResponse.status === 401 && originalRequest) {
        if (errorResponse.data.code === 'JWT_TOKEN_EXPIRED' && !originalRequest._retry) {
          if (isRefreshing) {
            return new Promise((resolve) => {
              refreshSubscribers.push((newToken) => {
                ...
                resolve(newApiManager(originalRequest));
              });
            });
          }
          ...
          try {
            const response = await newApiManager.post('/auth/refresh-token');
            ...
            return newApiManager(originalRequest);
          } catch (refreshError) {
