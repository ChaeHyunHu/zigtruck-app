...
export default function SettingsScreen() {
  ...
  const displayName = useMemo(() => profile?.name || '직트럭 사용자', [profile?.name]);
    ...
    return (
      <Screen variant="stack" className="flex-1 bg-gray50">
        ...
        <View className="flex-1 items-center justify-center gap-3">
          <Text className="text-[15px] text-gray700">로그인 후 이용 가능한 메뉴입니다.</Text>
          <Pressable
      ...
      <ScrollView className="flex-1">
        <View className="mx-4 mb-3 mt-3 flex-row items-center justify-between rounded-2xl border border-border bg-white p-4">
          <View className="flex-1">
            <Pressable className="flex-row items-center gap-1.5" onPress={onPressEditName}>
              <Text className="text-[22px] font-bold text-black">{displayName}</Text>
              <Ionicons name="pencil" size={18} color={appColors.gray800} />
            </Pressable>
            <Text className="mt-1 text-[18px] text-gray700">{displayPhone}</Text>
          </View>
          <View className="h-[70px] w-[70px] items-center justify-center">
            <View className="h-16 w-16 items-center justify-center overflow-hidden rounded-full border border-gray300 bg-white">
              {profile?.profileImageUri ? (
                <Image source={{ uri: profile.profileImageUri }} className="h-full w-full" />
              ) : (
        ...
        <View className="bg-white">
          <ListRow title="휴대폰 번호 변경" onPress={() => router.push('/settings/phone')} />
          <ListRow title="비밀번호 변경" onPress={() => router.push('/settings/password')} />
          <ListRow title="알림 설정" onPress={() => router.push('/notification-settings')} />
        </View>

        <View className="mt-2 bg-white">
          <Pressable className="min-h-14 flex-row items-center gap-1.5 px-4" onPress={onLogout}>
      ...
      <Modal visible={isNameModalOpen} transparent animationType="fade" onRequestClose={() => setIsNameModalOpen(false)}>
        <View className="flex-1 items-center justify-center bg-black/35 p-5">
          <View className="w-full rounded-[14px] bg-white p-4">
            <Text className="mb-3 text-[18px] font-bold text-gray900">이름 변경</Text>
            <TextInput
              value={editingName}
